...
@app.route('/shows/<int:id>/actors', methods=['GET', 'POST'])
def get_show_actors(id):
    if request.method == 'GET':
        show = queries.get_show_by_id(id)
        characters = queries.get_show_characters(id)
        show['characters_str'] = ', '.join([character['character_name'] for character in characters])
        show['actor_str'] = ', '.join([character['name'] for character in characters])
        show['characters'] = list([character['character_name'] for character in characters])
        show['actors'] = list([character['name'] for character in characters])
        show['pairs'] = dict(zip(show['characters'], show['actors']))
        print(show['pairs'])
        return render_template('show_character.html',
                                show=show)
    else:
